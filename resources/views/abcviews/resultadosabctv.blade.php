...
<h3 class="mt-5  mb-3">Videos de: {{$buscar}} </h3>
		...
		@foreach($videos as $vid )
		   ...
		   <div class="row mb-3">
		   	<div class="col-sm-3 text-center" style="background-image: url('{{asset('').$vid->thumbnail}}'); background-size: cover; max-height: 250px; ">
		   		<div><i class="fas fa-play mt-5  mb-5 " style="font-size: 40px;"></i></div>
		   	</div>
		   	<div class="col-sm-9">
		   			<h5 class="text-dark font-weight-bold">{{$vid->titulo}}</h5>
		   				<p class="text-muted truncated ">{{$vid->descripcion}}</p>
